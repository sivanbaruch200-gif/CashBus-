...
-- Database Schema Migration (FIXED VERSION)
-- Created: 2026-01-03
-- Fixed: Column reference issues, proper IF NOT EXISTS checks
-- =====================================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.claims ADD COLUMN workflow_status TEXT DEFAULT 'not_started' CHECK (workflow_status IN ('not_started', 'in_progress', 'completed', 'failed'));
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.claims ADD COLUMN priority TEXT DEFAULT 'normal' CHECK (priority IN ('urgent', 'high', 'normal', 'low'));
  END IF;
END $$;

-- Add user_id column to admin_users if it doesn't exist (for RLS policies)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    ...
    -- If user_id doesn't exist, add it (it should already be 'id' based on existing schema)
    -- We'll create an alias for the RLS policies
    ALTER TABLE public.admin_users ADD COLUMN user_id UUID;
    -- Copy id to user_id for existing rows
